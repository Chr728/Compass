...
describe("should test the upload images", () => {
  it("test to upload medication image", async () => {
    ...
    jest
      .spyOn(db.Medication, "update")
      .mockResolvedValueOnce(updatedMedicationImage);
    const res = await request(app)
    ...
    expect(res.body.message).toStrictEqual(
      "Medication image uploaded successfully"
    );
  });
...
describe("should test the updating image", () => {
  it("test to upload medication image", async () => {
    ...
    jest
      .spyOn(db.Medication, "update")
      .mockResolvedValueOnce(updatedMedicationImage2);
    const res = await request(app)
  ...
  afterAll(() => {
    fs.renameSync(
      "medicationImages/1_1_tylenol.jpeg",
      "medicationImages/1_tylenol.jpeg"
    );
  });
});
