...
export default function Appointment({
	...
	return (
		data && (
			<div className="bg-eggshell min-h-screen flex flex-col appointment mb-[60px]">
				...
				<div
					className="min-h-[500px] rounded-3xl bg-white flex flex-col  mt-4 w-full p-8 
                    shadow-[0_32px_64px_0_rgba(44,39,56,0.08),0_16px_32px_0_rgba(44,39,56,0.04)]">
					<div>
						<SingleEntry
							label={"Date:"}
							value={formatDate(data.date)}></SingleEntry>
					</div>
					<div>
						<SingleEntry
							label={"Time:"}
							value={formatMilitaryTime(data.time)}></SingleEntry>
					</div>

					<div>
						<SingleEntry
							label={"Appointment:"}
							value={data.appointmentWith}></SingleEntry>
					</div>

					<div>
						<SingleEntry
							label={"Reason:"}
							value={data.reason}></SingleEntry>
					</div>

					<div>
						<SingleEntry
							label={"Notes:"}
							value={data.notes}></SingleEntry>
					</div>
