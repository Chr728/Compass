...
export default function GetActivityJournal({
	...
	return (
		<div className="bg-eggshell min-h-screen flex flex-col">
			<SpanHeader
				...
				headerText="View the Activity Journal"></SpanHeader>
			{activity && (
				<span className="rounded-2xl  mt-6 mb-10 mr-28 bg-white flex flex-col m-auto w-full md:max-w-[800px] md:min-h-[600px] p-8 shadow-[0_32px_64px_0_rgba(44,39,56,0.08),0_16px_32px_0_rgba(44,39,56,0.04)]">
					<div className="mt-3 relative">
						<SingleEntry
							label={"Date:"}
							value={formatDate(activity.date)}></SingleEntry>
						<SingleEntry
							label={"Time:"}
						...
						<SingleEntry
							label={"Activity:"}
							value={activity.activity}></SingleEntry>
						<SingleEntry
							label={"Duration(min):"}
							value={activity.duration}></SingleEntry>
						<SingleEntry
							label={"Notes:"}
							value={activity.notes}></SingleEntry>
					</div>
					<div className="mt-10 pb-4 self-center">
						<Button
							...
							onClick={() =>
								router.push(
									`/getActivityJournals/${activityJournal}/${activityJournal}`
								)
							}
						/>
						<Button
