...
export default function GetMedication({
  ...
  useEffect(() => {
    ...
    if (medicationdata?.image) {
      checkImage(`${process.env.NEXT_PUBLIC_API_URL}/${medicationdata.image}`);
    }
      ...
      {medicationdata && (
        <span className="rounded-2xl  mt-6 mb-10 mr-28 bg-white flex flex-col m-auto w-full md:max-w-[800px] md:min-h-[600px] p-8 shadow-[0_32px_64px_0_rgba(44,39,56,0.08),0_16px_32px_0_rgba(44,39,56,0.04)]">
          <div className="mt-3 relative">
            ...
            <SingleEntry
              label={"Start Date:"}
              value={formatDate(medicationdata.dateStarted)}
            ></SingleEntry>
            <SingleEntry
              label={"Expiration Date:"}
              value={formatDate(medicationdata.expirationDate)}
            ></SingleEntry>
            <SingleEntry
              label={"Time:"}
              value={formatMilitaryTime(medicationdata.time)}
            ></SingleEntry>
            <SingleEntry
              label={"Dosage:"}
          ...
          <div className="mt-10 pb-4 self-center">
            <Button
              ...
              style={{ width: "140px" }}
              onClick={() =>
                router.push(`/getMedications/${medication}/${medication}`)
              }
            />
            <Button
              type="button"
