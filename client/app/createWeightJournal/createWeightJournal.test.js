...
describe("weight journal tests", () => {
	...
	it("weight journal entry is created", async () => {
		...
		const result = await createWeightJournal(
			mockUserId,
			mockWeightJournalData
		);
	...
	it("Height or weight cant be zero", async () => {
		...
		expect(weightError.textContent).toBe(
		...
		const heightError = screen.getByLabelText(
	...
	it("Height or weight cant be negative", async () => {
		...
		await waitFor(() => {
			const weightError =
				screen.getByLabelText("Weight").nextElementSibling;
			expect(weightError.textContent).toBe(
				"You can't enter a negative weight or a weight of zero."
			);
		});
		...
		await waitFor(() => {
			...
			expect(heightError.textContent).toBe(
				"You can't enter a negative height or a height of zero."
			);
		});
	});
