...
export default function RecordAudioPage() {
	...
	useEffect(() => {
		async function fetchAudioEntries() {
			try {
				...
				console.log("CHRISTINA", entryData.data);
    ...
    const WaveformView = ( { entries}) => {
  const [peaksInstance, setPeaksInstance] = useState(null);

  useEffect(() => {
    if (entries && entries.length > 0) {
      // Assuming that the audio file URLs are stored in the 'entries' array
      const audioUrls = entries.map((entry: { audioUrl: any; }) => entry.audioUrl);

      // Initialize Peaks.js instance
      const options = {
        containers: {
      ...
      const peaks = Peaks.init(options, function(err, peaksInstance) {
        if (err) {
          console.error('Failed to initialize Peaks.js', err);
          return;
        }
        console.log('Peaks.js initialized');
  ...
  useEffect(() => {
    // Your logic to highlight parts of the waveform based on AI predictions
    // You can access the Peaks.js instance using 'peaksInstance'
    // Example:
    if (peaksInstance) {
      // Assuming your AI predictions are stored in 'predictions'
      predictions.forEach(prediction => {
        if (prediction.value === 1) {
          peaksInstance.segments.add({ startTime: prediction.startTime, endTime: prediction.endTime, color: 'green' });
        } else {
          peaksInstance.segments.add({ startTime: prediction.startTime, endTime: prediction.endTime, color: 'red' });
        }
      });
    }
  }, [peaksInstance, predictions]);

	// useEffect(() => {
	// 	if (recordedAudioBlob) {
	// 		initPeaks(recordedAudioBlob);
	// 	}
	// }, [recordedAudioBlob]);
const YourComponent = ({ user, itemRecorded }) => {
	...
	return (
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <WaveformView entry={selectedEntry} />
          </div>
        </div>
      )}
					</>
