...
export async function getMedications(): Promise<any> {
	try {
		...
		if (response.ok) {
			logger.info(
				`medication entries fetched successfully for user ${id}`
			);
...
export async function createMedication(medicationData: any): Promise<any> {
	try {
		const currentUser = auth.currentUser;
		if (!currentUser) {
			logger.error("No user is currently signed in.");
...
export async function deleteMedication(medicationId: string): Promise<any> {
	try {
		const currentUser = auth.currentUser;
		if (!currentUser) {
			logger.error("No user is currently signed in.");
		...
		logger.info(
			`medication intake journal entry ${medicationId} deleted successfully`
		);
		if (response && response.status && !response.ok) {
			logger.error(
				`Failed to delete Medication entry. HTTP Status: ${response.status}`
			);
			throw new Error(
				`Failed to delete Medication entry. HTTP Status: ${response.status}`
			);
			...
			logger.info(
				`Medication entry ${medicationId} deleted successfully`
			);
